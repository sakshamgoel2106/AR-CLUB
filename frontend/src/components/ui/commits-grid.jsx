...
export const CommitsGrid = ({ text }) => {
  ...
  const getRandomFlash = () => +(Math.random() < 0.1); // lower flash chance

  return (
    <section
      className="w-full max-w-xl bg-[var(--bg-0)] border border-white/10 grid p-1.5 sm:p-3 gap-0.5 sm:gap-1 rounded-[10px] sm:rounded-[15px] shadow-2xl"
      style={{
      ...
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={index}
            className={cn(
              `h-full w-full aspect-square rounded-[2px] sm:rounded-[3px]`,
              isHighlighted ? "animate-highlight" : "",
              shouldFlash ? "animate-flash" : "",
              !isHighlighted && !shouldFlash ? "bg-[var(--bg-100)]" : ""
            )}
            style={{
